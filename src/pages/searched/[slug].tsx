import { setCuisine } from '@/store/slices/cuisineFilters.slice';
import React, { useEffect } from 'react';
import RecipesList from '@/components/recipes/RecipesList';
import { useFetchRecipes } from '@/services/fetchRecipes';
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import { setSearchText, setPage } from '@/store/slices/cuisineFilters.slice';
import { Group, Pagination, Skeleton } from '@mantine/core';
import { useRouter } from 'next/router';

const SearchedPage = () => {
  const router = useRouter();
  const searchText = router.query.slug?.toString() ?? '';
